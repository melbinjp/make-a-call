...
function reconnectFirebaseTemporarily() {
  if (!isConnectedToFirebase && database) {
    this.logEvent(
      'Firebase',
      'Temporarily reconnecting for signaling operation.'
    );
...
class PhoneCall {
  ...
  setupEventListeners() {
    if (this.isSafari) {
      this.elements.callSetup.innerHTML =
        '<p style="padding: 20px; text-align: center;">Safari is not fully supported for WebRTC features. Please use Chrome or Firefox for the best experience.</p>';
    ...
    if (this.elements.p2pConnectBtn)
      this.elements.p2pConnectBtn.addEventListener('click', () =>
        this.showP2PModal()
      );
    ...
    if (modeToggle)
      modeToggle.addEventListener('click', () => this.toggleConnectionMode());
    if (modeInfoBtn)
      modeInfoBtn.addEventListener('click', () => this.showModeInfoModal());
    ...
    if (this.elements.copyMyP2PBtn)
      this.elements.copyMyP2PBtn.addEventListener('click', () =>
        this.copyMyP2PUrl()
      );
    ...
    if (this.elements.startCallBtn)
      this.elements.startCallBtn.addEventListener('click', () =>
        this.startCall()
      );
    ...
    if (this.elements.participantsToggle)
      this.elements.participantsToggle.addEventListener('click', () =>
    ...
    if (this.elements.leaveGroupBtn)
      this.elements.leaveGroupBtn.addEventListener('click', () =>
        this.leaveRoom()
      );
    if (this.elements.renameGroupBtn)
      this.elements.renameGroupBtn.addEventListener('click', () =>
        this.renameRoom()
      );
    if (this.elements.groupMenuBtn)
      this.elements.groupMenuBtn.addEventListener('click', (e) => {
        e.stopPropagation();
    ...
    if (snapshotStatsBtn)
      snapshotStatsBtn.addEventListener('click', () => this.snapshotStats());
    ...
    if (isMobile && !isSmartwatch) {
      ...
      if (this.elements.nameInput) {
        this.elements.nameInput.addEventListener('focus', () => {
          setTimeout(
            () =>
              this.elements.nameInput.scrollIntoView({
                behavior: 'smooth',
                block: 'center',
              }),
            300
    ...
    try {
      ...
      if (error.name === 'NotAllowedError') {
        ...
        this.showNotification(
          'Could not start call. Please check microphone permissions.',
          'error'
        );
      }
    }
  ...
  updateButtonStates() {
    ...
    if (this.isCallActive) {
      ...
      this.elements.startCallBtn.innerHTML =
        '<i class="fas fa-phone"></i><span class="btn-text">Join</span>';
  ...
  updateParticipantCallStatus() {
    if (database && this.channel && this.deviceHash) {
      database
        .ref(`channels/${this.channel}/participants/${this.deviceHash}/inCall`)
        .set(this.isCallActive);
    }
  }
  ...
  async requestRoomAccess(channelId, userName) {
    ...
    const requestRef = database
      .ref(`channels/${channelId}/joinRequests`)
      .push();
  ...
  setupAccessRequestListener() {
    ...
    database
      .ref(`channels/${this.channel}/joinRequests`)
      .on('child_added', (snapshot) => {
        const requestData = snapshot.val();
        if (
          requestData &&
          requestData.status === 'pending' &&
          requestData.deviceHash !== this.deviceHash
        ) {
  ...
  createPeerConnection(peerId) {
    ...
    pc.onicecandidate = (event) => {
      try {
        ...
        this.logEvent(
          'ICE',
          `Error gathering candidate for ${peerId}: ${error.message}`,
          null,
          'error'
        );
    ...
    pc.ontrack = (event) => {
      try {
        console.log(
          `[${this.userName}] 🎵 Received track from`,
          sanitizeForLog(peerId)
        );
    ...
    pc.onsignalingstatechange = () => {
      this.logEvent(
        'Signaling',
        `State changed for ${peerId}: ${pc.signalingState}`
      );
      console.log(
        `[${this.userName}] WebRTC signaling state change for peer ${peerId}: ${pc.signalingState}`
      );
    };
    ...
    pc.oniceconnectionstatechange = () => {
      this.logEvent(
        'ICE',
        `Connection state changed for ${peerId}: ${pc.iceConnectionState}`
      );
      console.log(
        `[${this.userName}] WebRTC ICE connection state change for peer ${peerId}: ${pc.iceConnectionState}`
      );
    };
    ...
    pc.onnegotiationneeded = async () => {
      try {
        ...
        if (
          ...
          console.log(
            `[${this.userName}] onnegotiationneeded triggered for peer ${peerId}, sending offer.`
          );
          await this.sendOffer(peerId);
        }
      } catch (err) {
        console.error(
          `Error during onnegotiationneeded for peer ${peerId}:`,
          err
        );
      }
    };
    ...
    pc.onconnectionstatechange = () => {
      this.logEvent(
        'Connection',
        `State changed for ${peerId}: ${pc.connectionState}`
      );
      console.log(
        `[${this.userName}] WebRTC connection state change for peer ${peerId}: ${pc.connectionState}`
      );
      const state = this.peerConnectionStates.get(peerId);
      ...
      if (pc.connectionState === 'connected') {
        ...
        if (state && !state.iceRestarted) {
          this.logEvent(
            'ICE',
            `Connection to ${peerId} failed. Attempting to restart ICE.`,
            null,
            'warning'
          );
          ...
          this.logEvent(
            'ICE',
            `ICE restart failed for ${peerId}. Attempting full renegotiation.`,
            null,
            'error'
          );
          if (!state.polite) {
        ...
        if (this.connectedPeers.size === 0 && this.channel) {
          this.logEvent(
            'Firebase',
            'All P2P connections lost, reconnecting to signaling server.'
          );
    ...
    dataChannel.onopen = () => {
      try {
        ...
        this.logEvent(
          'DataChannel',
          `Error on open for ${peerId}: ${error.message}`,
          null,
          'error'
        );
    ...
    dataChannel.onerror = (error) => {
      this.logEvent(
        'DataChannel',
        `Channel to ${peerId} error: ${error.message}`,
        null,
        'error'
      );
  ...
  checkUrlParams() {
    ...
    if (p2pData) {
      try {
        ...
        this.showNotification(
          'Invalid P2P link. It might be malformed or expired.',
          'error'
        );
      }
    }
  ...
  showReconnectionOption(session) {
    ...
    this.elements.callSetup.insertBefore(
      reconnectDiv,
      this.elements.callSetup.firstChild
    );
  ...
  async createRoom() {
    ...
    if (!userName || !validateInput(userName, 'text', 30)) {
      this.showNotification(
        'Please enter a valid name (letters, numbers, spaces only)',
        'error'
      );
      return;
    }
  ...
  async joinGroup() {
    ...
    if (!roomCode || !validateInput(roomCode, 'roomCode', 20)) {
      this.showNotification(
        'Enter valid group code (letters, numbers, dashes only)',
        'error'
      );
      return;
    }
    ...
    try {
      ...
      this.showNotification(
        'Could not join group. Please check the code and your connection.',
        'error'
      );
    }
  }
  ...
  async sendMessage() {
    ...
    if (
      !message ||
      !validateInput(message, 'text', 200) ||
  ...
  displayMessage(messageData) {
    ...
    const senderAlias = isOwnMessage
      ? this.getContactAlias(this.deviceHash)
  ...
  async copyMyP2PUrl() {
    ...
    try {
      ...
      this.showNotification(
        'Direct connect URL copied. Share it with a friend!',
        'success'
      );
  ...
  async handleP2PMessage(data, senderId) {
    try {
      ...
      if (messageData.data?.signature && messageData.data?.contactId) {
        if (
          !this.verifyPeerIdentity(
            messageData.data.contactId,
            messageData.data.signature
          )
        ) {
      ...
      if (messageData.type === 'speaking') {
        this.updateParticipantSpeakingState(
          messageData.senderHash,
          messageData.speaking
        );
        return;
      }
  ...
  handlePeerIntroduction(data, introducerId) {
    ...
    console.log(
      `Introduced to new peer: ${newPeer.userName} by ${introducerId}`
    );
  }
  ...
  async joinP2PNetwork(knownPeerInfo) {
    try {
      ...
      const pc = await this.createDirectConnection(
        knownPeerInfo.userName,
        knownPeerInfo
      );
  ...
  async attemptDirectReconnect() {
    ...
    const peers = Object.entries(network).filter(
      ([peerId]) => peerId !== this.userName
    );
  ...
  sendDirectSignal(peerId, type, data) {
    ...
    if (channel && channel.readyState === 'open') {
      channel.send(
        JSON.stringify({ type: `direct-${type}`, data, from: this.userName })
      );
    }
  }
  ...
  async loadChatHistory() {
    try {
      ...
      if (rooms.length === 0) {
        this.elements.historyList.innerHTML =
          '<div class="history-item">No recent groups found.</div>';
        return;
      }
      ...
      this.elements.historyList.innerHTML =
        '<div class="history-item">Could not load group history.</div>';
    }
  }
  ...
  async viewRoomHistory(channel, displayName) {
    try {
      ...
      secureNotify(
        `Last messages from ${displayName}: ${messageText.substring(0, 100)}...`,
        'info'
      );
    } catch (e) {
  ...
  async joinChannel() {
    ...
    if (firebaseEnabled) {
      ...
      const network = this.loadPersistentNetwork();
      const hasPersistentPeers = Object.keys(network).some(
        (peerId) => peerId !== userName
      );
      ...
      if (hasPersistentPeers) {
        ...
        if (reconnectSuccess && this.connectedPeers.size > 0) {
          ...
          this.showNotification(
            'Reconnected to persistent network!',
            'success'
          );
          return;
        }
  ...
  async joinViaFirebase() {
    ...
    try {
      ...
      const nameAvailable = await this.checkNameAvailability(
        this.channel,
        this.userName
      );
      if (!nameAvailable) {
        const snapshot = await database
          .ref(`channels/${this.channel}/participants`)
          .once('value');
  ...
  async getParticipantCount(channelId) {
    try {
      const snapshot = await database
        .ref(`channels/${channelId}/participants`)
        .once('value');
      ...
      console.error(
        ...
        sanitizeForLog(error.message || error)
      );
      return 0;
    }
  }
  ...
  async checkNameAvailability(channelId, userName) {
    try {
      const snapshot = await database
        .ref(`channels/${channelId}/participants/${userName}`)
        .once('value');
      return !snapshot.exists();
    } catch (error) {
  ...
  async getUserMedia() {
    try {
      ...
      try {
        ...
        console.error(
          'Audio processing setup failed:',
          sanitizeForLog(audioError.message)
        );
      }
      ...
      this.peerConnections.forEach((pc) => {
        try {
          ...
          console.error(
            ...
            sanitizeForLog(trackError.message)
          );
        }
      });
  ...
  setupSignaling(isNewRoom = false) {
    ...
    database
      ...
      .on('value', (snapshot) => {
        ...
        this.othersInCall = Object.values(participants).some(
          (p) => p.inCall && p.deviceHash !== this.deviceHash
        );
        this.updateButtonStates();
        ...
        const newHashes = currentHashes.filter(
          (hash) => hash !== this.deviceHash && !this.peerConnections.has(hash)
        );
        ...
        newHashes.forEach((peerHash) => {
          if (participants[peerHash] && !this.peerConnections.has(peerHash)) {
            console.log(
              `[${this.userName}] Found new peer: ${peerHash}. Creating PeerConnection.`
            );
            const polite = this.deviceHash > peerHash;
            this.peerConnectionStates.set(peerHash, {
              isMakingOffer: false,
              polite: polite,
            });
            ...
            console.log(
              `[${this.userName}] Peer connection created for ${peerHash}. I am ${polite ? 'polite' : 'impolite'}.`
            );
          }
        });
  ...
  async sendOffer(peerId) {
    ...
    try {
      ...
      if (!pc) {
        console.error(
          `[${this.userName}] No peer connection found for ${peerId} when trying to send offer.`
        );
        return;
      }
      ...
      console.log(
        `[${this.userName}] 🚀 Sent offer to peer:`,
        encodeURIComponent(peerId)
      );
    } catch (error) {
      console.error(
        `[${this.userName}] Error creating offer for peer:`,
        encodeURIComponent(peerId),
        error.message || error
      );
    } finally {
  ...
  async handleSignal(data) {
    try {
      ...
      if (
        payload &&
        payload.targetPeer &&
        payload.targetPeer !== this.deviceHash
      ) {
        return;
      }
      ...
      if (!payload) {
        console.warn(
          `[${this.userName}] Received signal of type ${type} with no payload.`
        );
        return;
      }
      ...
      switch (type) {
        case 'offer':
          ...
          if (!payload.offer || !payload.offer.type || !payload.offer.sdp) {
            console.warn(
              `[${this.userName}] Received invalid offer payload:`,
              payload
            );
            return;
          }
          ...
          if (ignoreOffer) {
            console.warn(
              `[${this.userName}] Glare detected and ignoring offer from peer ${sender}. I am impolite.`
            );
            return;
          }
          ...
          if (this.iceCandidateQueue.has(sender)) {
            ...
            this.logEvent(
              'ICE',
              `Processing ${queue.length} queued candidates from ${sender}.`
            );
          ...
          console.log(
            `[${this.userName}] 📞 Answered call from peer:`,
        ...
        case 'answer':
          ...
          if (!payload.answer || !payload.answer.type || !payload.answer.sdp) {
            console.warn(
              `[${this.userName}] Received invalid answer payload:`,
              payload
            );
            return;
          }
          ...
          if (answerPc && answerPc.signalingState === 'have-local-offer') {
            ...
            console.log(
              `[${this.userName}] ✅ Call established with peer:`,
              encodeURIComponent(sender)
            );
          } else {
            console.warn(
              `[${this.userName}] Received answer from ${sender} but not in have-local-offer state.`
            );
          }
          break;
        ...
        case 'ice-candidate':
          ...
          if (!payload.candidate) {
            console.warn(
              `[${this.userName}] Received invalid ICE candidate payload:`,
              payload
            );
            return;
          }
          ...
          if (candidatePc) {
            if (candidatePc.remoteDescription) {
              ...
              this.logEvent(
                'ICE',
                `Queued candidate from ${sender} (remote description not set).`
              );
            }
          }
          break;
  ...
  addParticipant(deviceHash, name) {
    ...
    // Authorization check
    if (
      !validateInput(deviceHash, 'deviceHash') ||
      !validateInput(name, 'text', 30)
    ) {
    ...
    participantRef
      ...
      .catch((error) => {
        ...
        this.showNotification(
          'Connection failed - check Firebase setup',
          'error'
        );
      });
  ...
  removeParticipant(deviceHash) {
    ...
    database
      .ref(`channels/${this.channel}/participants/${deviceHash}`)
      .remove()
      .catch((error) => {
  ...
  updateParticipantsDisplay() {
    ...
    Object.values(this.participantsData).forEach((participant) => {
      ...
      if (
        ...
        console.warn(
          'Skipping invalid participant entry received from Firebase:',
          participant
        );
  ...
  updateParticipantSpeakingState(deviceHash, isSpeaking) {
    const participantItem = document.querySelector(
      `.participant-item[data-hash="${deviceHash}"]`
    );
    if (participantItem) {
      if (isSpeaking) {
  ...
  async editContactAlias(deviceHash) {
    ...
    const promptText = isMe
      ? 'Set your display name:'
      : 'Set alias for this contact:';

    const newAlias = prompt(promptText, currentAlias);

    if (newAlias !== null) {
      await this.setContactAlias(
        deviceHash,
        newAlias || (isMe ? 'me' : deviceHash.substring(0, 8))
      );
  ...
  showCallerLimitWarning() {
    ...
    this.elements.callInterface.insertBefore(
      warning,
      this.elements.callInterface.firstChild
    );
  }
  ...
  async playTestTone() {
    ...
    try {
      ...
      if (originalTrack) {
        const replacePromises = Array.from(this.peerConnections.values()).map(
          (pc) => {
            const sender = pc
              .getSenders()
  ...
  updateConnectionStatus() {
    ...
    if (totalPeers > 0) {
      ...
      const hasFailed = Array.from(this.peerConnections.values()).some(
      ...
      if (allConnected) {
        ...
        statusType = 'error';
        if (this.elements.statusDot)
          this.elements.statusDot.classList.add('failed');
      } else {
  ...
  async createP2PRoom() {
    ...
    this.showCallInterface();
    this.showNotification(
      'Direct Group Created. Share the link to invite others.',
      'info'
    );
    this.generateShareUrl();
    this.elements.shareSection.style.display = 'block';
  ...
  async loadContacts() {
    const contacts =
      (await this.loadFromEncryptedStorage('recentContacts')) || [];
  ...
  displayContacts(contacts) {
    ...
    if (contacts.length === 0) {
      this.elements.contactsList.innerHTML =
        '<p style="text-align: center; color: var(--text-secondary);">No contacts yet</p>';
      return;
    }
  ...
  toggleConnectionMode() {
    ...
    if (toggle.classList.contains('cloud-mode')) {
      ...
      this.showNotification(
        'Direct Connection Mode: Only P2P links will work.',
        'info'
      );
    } else {
      ...
      this.showNotification(
        'Group Mode: Connect easily with group codes.',
        'info'
      );
    }
  }
  ...
  async setContactAlias(deviceHash, alias) {
    this.contactAliases[deviceHash] =
      alias.trim() ||
      (deviceHash === this.deviceHash ? 'me' : deviceHash.substring(0, 8));
  ...
  async addContact(deviceHash, displayName) {
    if (!this.contactAliases[deviceHash]) {
      this.contactAliases[deviceHash] =
        displayName || deviceHash.substring(0, 8);
      await this.saveContactAliases();
    }
  }
  ...
  showFirebaseToggle() {
    ...
    if (toggle) {
      ...
      toggle.className = firebaseEnabled
        ? 'firebase-toggle enabled'
        : 'firebase-toggle disabled';
    }

    const shareLabel = document.getElementById('shareLabel');
    if (shareLabel) {
      shareLabel.textContent = firebaseEnabled
        ? 'Share Room (Firebase)'
        : 'Share Room (P2P)';
    }
  }
  ...
  toggleFirebase() {
    if (firebaseEnabled) {
      ...
      database = initializeFirebase();
      this.showNotification(
        'Firebase enabled - Server connections available',
        'info'
      );
    }
    this.showFirebaseToggle();
  ...
  processP2PUrl(url) {
    try {
      // Use a dummy base if the URL is just a query string like "?p=..."
      ...
      this.showNotification(
        'Invalid P2P URL. It might be malformed or expired.',
        'error'
      );
    }
  }
  ...
  findPeerByContactId(contactId) {
    try {
      const channels = Object.keys(localStorage).filter((k) =>
        k.startsWith('p2pNetwork_')
      );
  ...
  async introduceToAllPeers(newPeerInfo) {
    ...
    await database
      .ref(`channels/${this.channel}/introductions`)
      .push(introductionData);
      ...
      try {
        ...
        if (selectedPair) {
          ...
          if (localCandidate && remoteCandidate) {
            this.logEvent(
              'Stats',
              `  - Local: ${localCandidate.address}:${localCandidate.port} (${localCandidate.candidateType})`
            );
            this.logEvent(
              'Stats',
              `  - Remote: ${remoteCandidate.address}:${remoteCandidate.port} (${remoteCandidate.candidateType})`
            );
            this.logEvent('Stats', `  - Transport: ${localCandidate.protocol}`);
            this.logEvent(
              'Stats',
              `  - RTT: ${selectedPair.currentRoundTripTime}s`
            );
          }
        } else {
        ...
        this.logEvent(
          'Stats',
          `Could not get stats for peer ${peerId}: ${err.message}`,
          null,
          'error'
        );
      }
